...
function ItemStatusButton({ item, orderId, onUpdate, disabled }) {
  const [updating, setUpdating] = useState(false)
  const flow = ITEM_STATUS_FLOW[item.status]

  const handleClick = async () => {
    if (!flow.next || updating || disabled) return
    setUpdating(true)
    try {
      const res = await api.put(`/orders/${orderId}/items/${item.id}/status`, { status: flow.next })
  ...
  return (
    <button
      onClick={handleClick}
      disabled={!flow.next || updating || disabled}
      style={{
        background: st.bg,
        color: st.color,
        border: 'none',
        ...
        fontSize: '.82rem',
        fontWeight: 700,
        cursor: flow.next ? 'pointer' : 'default',
        opacity: updating ? .6 : 1,
        transition: 'opacity .15s, transform .1s',
        whiteSpace: 'nowrap',
      }}
    >
      {updating ? '...' : (flow.next ? flow.label : ITEM_STATUS_LABELS[item.status])}
    </button>
  )
}
...
function OrderTicket({ order, onOrderUpdate }) {
  ...
  const isAlert = minutes >= ALERT_MINUTES
  const allListo = order.order_items?.every(i => i.status === 'listo')
  const statusColor = ORDER_STATUS_COLORS[order.status]

  return (
    <div style={{
      ...ts.ticket,
      borderColor: isAlert ? 'var(--coral)' : allListo ? 'var(--success)' : order.status === 'pendiente' ? 'var(--warning)' : '#1565c0',
      animation: isAlert ? 'alertPulse 1.5s ease-in-out infinite' : 'none',
    }}>
      {/* Ticket header */}
      <div style={ts.ticketHeader}>
        <div style={ts.mesaNum}>{order.table_name || `Mesa ${order.table_number}`}</div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '.15rem' }}>
          <OrderTimer createdAt={order.created_at} />
          {isAlert && (
            <span style={{ fontSize: '.72rem', color: 'var(--coral)', fontWeight: 700 }}>
              ⚠️ TIEMPO EXCEDIDO
            </span>
          )}
        </div>
      ...
      <div style={ts.meseroLabel}>👤 {order.waiter_name}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
        {order.order_items?.map(item => (
          <div key={item.id} style={{
            ...ts.itemRow,
            background: item.status === 'listo' ? '#e6f4ea'
              : item.status === 'en_preparacion' ? '#e3f2fd'
              : 'var(--gray-50)',
            opacity: item.status === 'listo' ? .75 : 1,
          }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '1rem' }}>
                <span style={{ color: 'var(--orange)', marginRight: '.25rem' }}>{item.quantity}x</span>
                {item.dish_name}
              </div>
              {item.notes && (
      ...
      {/* Footer del ticket */}
      {allListo && (
        <div style={ts.readyBanner}>
          🔔 ¡ORDEN LISTA! — Notificando al mesero
        </div>
      )}
    </div>
  )
...
export default function Cocina() {
  ...
  const handleOrderUpdate = (updatedOrder) => {
    ...
    setOrders(prev => {
      ...
      // Si la orden pasó a 'lista', mantenerla para que el chef vea
      if (['cobrada','cancelada'].includes(updatedOrder.status)) {
  ...
  const filtered = orders.filter(o => {
    ...
    if (filter === 'pendiente') return o.status === 'pendiente'
    if (filter === 'preparacion') return o.status === 'en_preparacion'
    if (filter === 'listas') return o.status === 'lista'
    return true
  })
  ...
  const alertOrders = orders.filter(o => {
    ...
    return mins >= ALERT_MINUTES && o.status !== 'lista'
  })
    ...
    <div style={cs.fullBg}>
      ...
      {alertOrders.length > 0 && (
        <div style={cs.alertBanner}>
          ...
          {alertOrders.map(o => o.table_name || `Mesa ${o.table_number}`).join(', ')}
        </div>
      )}
...
// Añadir keyframes de alerta al CSS global si no están
